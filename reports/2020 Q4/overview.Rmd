...
- `r overview$partial` New programs
- `r overview$full` Established programs (data for 12 months)
...
gamut_log <- 
    readr::read_csv(here::here("data/Logs/GAMUTDatabase_Logging.csv"), 
                            col_types = cols(
...
AIM_log <- 
    readr::read_csv(here::here("data/Logs/AIMDatabase_Logging.csv"), 
                            col_types = cols(
                                timestamp = col_datetime(format = ""),
                                Username = col_character(),
                                Action = col_character(),
...
AEL_log <- 
    readr::read_csv(here::here("data/Logs/AELDatabase_Logging.csv"), 
                            col_types = cols(
                                timestamp = col_datetime(format = ""),
                                Username = col_character(),
                                Action = col_character(),
...
MTR_log <- 
    readr::read_csv(here::here("data/Logs/MTRDatabase_Logging.csv"), 
                            col_types = cols(
                                timestamp = col_datetime(format = ""),
                                Username = col_character(),
                                Action = col_character(),
...
log_data <-
    ...
    filter(grepl("Record", Action)) %>%
    #count(Action) %>% arrange(desc(n))
    ...
    #mutate_at(vars(Action, record, record_action, program), ~as.factor) %>% head()
    select(-4) %>%
    filter(!is.na(record) )

#gamut_log %>% filter(Username == "drakejoan@health.missouri.edu") %>% filter(grepl("Updated.*Oct 2020", Action)) %>% View()
#log_data %>% filter(Username == "drakejoan@health.missouri.edu") %>% filter(grepl("Updated.*Oct 2020", Action)) %>% View()
...
month_submissions_log <- 
    ...
    filter(record_action == "Updated Month") %>%
    select(program, month) %>% distinct() %>%
    group_by(program, month) %>%
    #slice(1) %>%
    group_by(month) %>%
    summarise(submitted = n())

#month_submissions_log %>% filter(month >= "2020-01-01") 
...
#stacked_barplot <- 
    ...
    scale_fill_manual(#values = c("light blue", "blue"),
        values = c("light green", "light grey"), 
        labels = c("Submitted data", "Enrolled" )) +
