...
message  <- "This GAMUT database report is respectfully distributed for your review. This report includes data between July 2016 to June 2017.

Thank you on behalf of the GAMUT QI oversight committee."

subject <- "GAMUT 2017 Q2 Summary Report"
org_subject <- "GAMUT 2017 Q2 Organizational Summary Report"
org_message <- "
This GAMUT database report for your organization is respectfully distributed for your review. This version shows the metric performance for each of your organization's programs on a single report.
...
for (row in 1:length(sendit_data$dm_email)) {
    ...
    response <-  sendit(username=username, password=password,
                        subject=paste("GAMUT 2017 Q2 Summary Report - ", sendit_data$program_name[row]),
                        message = message,
                        recipient =  sendit_data$dm_email[row],
                        expireDays = 15,
                        file = paste0("GAMUT_2017Q2_", sendit_data$file[row], ".pdf"),
                        url = url
    )
