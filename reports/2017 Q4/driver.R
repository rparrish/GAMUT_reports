...
#' driver_2017Q4.R
#'
#' generates a 2017 Q4 GAMUT report for each data access group
#'
#' Rmarkdown version
...
reports_data <- data.frame(
    ...
    program_name = c("AIM Native Air AZ-NM"), #c( "Airlift Northwest", "AEL IL", "Flight For Life CO",  "Mercy Life Line"),
...
for(org in organizations$org) {
    ...
    rmarkdown::render("GAMUT_DAG_template.Rmd",
                      ...
                      clean = TRUE,
                      envir = new.env(),
                      output_file =paste0('GAMUT_2017Q4_', filename, '_Org_Summary.pdf')
